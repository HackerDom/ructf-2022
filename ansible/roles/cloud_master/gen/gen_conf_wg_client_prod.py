...
try:
    os.mkdir("client_wg_prod")
except FileExistsError:
    print("Remove ./client_wg_prod dir first")
    sys.exit(1)

for i in range(1, N+1):
    srv_pub_key = open("net_keys_wg_prod/%d.srv.pubkey.txt" % i).read().strip()
    client = CLIENT_HEADER

    for c in range(1, C+1):
        clt_priv_key = open("net_keys_wg_prod/%d.clt%d.privkey.txt" % (i, c)).read().strip()
        client += CLIENT_INTERFACE_INFO % (clt_priv_key, 60+i//256, i%256, 200+c)

    client += CLIENT_FOOTER % (srv_pub_key, SERVER_NAME%i, 31000+i)

    open("client_wg_prod/%d.conf" % i, "w").write(client)

print("Finished, check ./client_wg_prod dir")
