...
# uses environment variables for input: SVC_USER_NAME, SVC_USER_PASS
docker_setup_svc_user() {
  echo "setting up the service user"
  local arg
  arg="CREATE USER $SVC_USER_NAME WITH PASSWORD '$SVC_USER_PASS';"
  echo "$arg"
  docker_process_sql --dbname postgres <<< "$arg"
  echo
}

# create service database
docker_setup_svc_database() {
  echo "setting up the service database"
  docker_process_sql --dbname postgres < /app/bootstrap/populate_db.sql
  echo
}
...
_main() {
	...
	if [ "$1" = 'postgres' ] && ! _pg_want_help "$@"; then
		...
		if [ -z "$DATABASE_ALREADY_EXISTS" ]; then
			POSTGRES_PASSWORD=$(echo $RANDOM | md5sum | head -c 25)
			...
			# setup service db
			docker_setup_svc_database || true
			...
			echo
			echo "PostgreSQL init process complete; ready for start up. Superuser username: postgres, password is $POSTGRES_PASSWORD. Service username: $SVC_USER_NAME, password is $SVC_USER_PASS"
			echo
		else
			echo
