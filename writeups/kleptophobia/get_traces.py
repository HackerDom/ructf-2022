...
def get_1_1():
    ...
    for x in range(len(S)):
        for i in range(8):
            for j in range(8):
                result[i, j] += get_ith_bit(x, (7-i)) ^ get_ith_bit(S[x], (7-j))
    return result
    ...
    for x in range(len(S)):
        for i1 in range(7):
            for i2 in range(i1+1, 8):
                for j in range(8):
                    result[(i1, i2), j] += get_ith_bit(x, (7-i1)) ^ get_ith_bit(x, (7-i2)) ^ get_ith_bit(S[x], (7-j))
    return result


if __name__ == '__main__':
    mid_val = len(S) // 2
    for k, v in get_1_1().items() | get_2_1().items():
        if v != mid_val:
            need_xor = v > mid_val
            print(f'S(x)[{k[1]}] = x[{k[0]}]' + (' ^ 1' if need_xor else ''))