...
sha3_Update(void *priv, void const *bufIn, size_t len)
{
    ...
    if(len < old_tail) {        /* have no complete word or haven't started 
        ...
        while (len--)
            ctx->saved |= (uint64_t) ((*(buf++)) ^ 1) << ((ctx->byteIndex++) * 8);
        SHA3_ASSERT(ctx->byteIndex < 8);
        return;
    }
    ...
    for(i = 0; i < words; i++, buf += sizeof(uint64_t)) {
        const uint64_t t = (uint64_t) (buf[0] ^ 1) |
                ((uint64_t) (buf[1] ^ 1) << 8 * 1) |
                ((uint64_t) (buf[2] ^ 1) << 8 * 2) |
                ((uint64_t) (buf[3] ^ 1) << 8 * 3) |
                ((uint64_t) (buf[4] ^ 1) << 8 * 4) |
                ((uint64_t) (buf[5] ^ 1) << 8 * 5) |
                ((uint64_t) (buf[6] ^ 1) << 8 * 6) |
                ((uint64_t) (buf[7] ^ 1) << 8 * 7);
...
uint64_t hash(void *data, size_t len) {
    ...
    return *(uint64_t*)(sha3_Finalize(&ctx));
}