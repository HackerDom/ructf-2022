...
for i in range(1, N):
    ...
    shutil.copyfile("client_entergame_dev/%d.conf" % i,
                    DB_DIR + "/team%d/client_entergame.ovpn" % i)
    shutil.copyfile("server_outside_dev/%d.conf" % i,
                    DB_DIR + "/team%d/server_outside.conf" % i)

    shutil.copyfile("openvpn_team_main_net_client_dev/%d.conf" % i,
                    DB_DIR + "/team%d/game_network.conf" % i)

    shutil.copyfile("passwds_dev/team%d_root_passwd.txt" % i,
                    DB_DIR + "/team%d/root_passwd.txt" % i)

    shutil.copyfile("tokens_hashed_dev/%d.txt" % i, DB_DIR + "/team%d/token_hash.txt" % i)

    root_passwd_filename = "passwds_dev/team%d_root_passwd.txt" % i
